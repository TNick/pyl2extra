...
class PyDotPrinter(object):
    ...
    def run(self):
        ...
        self.graph = pd.Dot(graph_type='digraph', rankdir="TB")
        self.ext_setup()
...
class PyDotIfElseCluster(PyDotExtension):
    ...
    def __init__(self, base, cond_highlight):
        super(PyDotIfElseCluster, self).__init__(base)
...
def pydotprint(fct,
    ...
    if cond_highlight:
        pr.extensions.append(PyDotIfElseCluster(pr, cond_highlight))
    pr.run()
...
def path_from_var(self, srctag):
    """
    Extract the trace from a Variable instance.

    Parameters
    ----------
    srcnode : gof.Variable
    ...
    result : list of strings
        The trace for when the Op was created.
    """
    result = self.path_from_tag(srctag)
    return result
